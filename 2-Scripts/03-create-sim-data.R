...
source("1-Functions/create_simulation-pattern.R")
source("1-Functions/create_simulation-species.R")
  ...
  for (j in seq_along(fract_dim)) {
    ...
    for (k in seq_along(n_random)) {
      ...
      simulation_pattern <- create_simulation_pattern(raster = simulation_habitat,
                                                      number_points = number_points, mu = 5,
...
suppoRt::save_rds(object = simulation_experiment_list, filename = "main-sim-experiment.rds", 
